...
export default function VectorAdditionChart({
  ...
  const vectorData = useMemo(() => {
    const placedRows = rows.filter((row) => row.sliderData);
    if (placedRows.length === 0) return null;

    // Convert each row to a vector (change from A to B)
    // x-axis: value dimension (valueB - valueA)
    // y-axis: joy dimension (joyB - joyA)
    // Scale from [0,1] slider range to [-1,1] chart range
    const vectors = placedRows.map((row) => {
      const sliderData = row.sliderData!;
      return {
        id: row.id,
        category: row.category,
        dx: (sliderData.valueB - sliderData.valueA) * 2,
        dy: (sliderData.joyB - sliderData.joyA) * 2,
      };
    });
    ...
    return (
      <div className="value-joy-chart">
        ...
        <p className="instruction-text">
          Rate at least one category using the sliders above to see the vector
          addition
        </p>
      </div>
    );

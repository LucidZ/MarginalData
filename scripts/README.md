...
### `generate_six_degrees_data.py`

Builds the bounded co-star graph for the "Six Degrees Of..." actor network visualization.
...
- Raw IMDb datasets auto-download to `data/six_degrees/imdb/` on first run (~1.3GB, gitignored)
...
python scripts/generate_six_degrees_data.py --min-votes 500000 --min-movies 1 --out pool-a --skip-tmdb
```

**Output**:
- File: `public/data/six-degrees-<out>.json` — `{ actors: [...], edges: [[a, b, weight], ...] }`
...
- Periodically to pick up IMDb's daily dataset refresh (delete `data/six_degrees/imdb/` first to force a re-download)

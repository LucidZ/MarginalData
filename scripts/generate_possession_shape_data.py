#!/usr/bin/env python3
"""
Generate Possession Shape Data for D3/React Visualization

For each starting-XI player, averages the location of every on-ball action
they're involved in, split by whether their own team had possession at that
moment ("offense") or not ("defense"). This is the same average-position
methodology public tools like Opta/WhoScored use (touch-based, not tracking
data) — StatsBomb's open 360 freeze-frame data can't improve on it here
because it only identifies the event's own actor by name; every other
visible player in a 360 frame is anonymized to a bare teammate/opponent
flag, so it can't add named off-ball samples for anyone else.

Usage:
    python scripts/generate_possession_shape_data.py
"""
...
OUT_PATH = "public/data/statsbomb/wc2022-final-possession-shape.json"
...
def main():
    ...
    for p in players:
        ...
        print(f"  {p['team']:10s} {p['fullName']:35s} off n={off_n:4d}  def n={def_n:4d}")
